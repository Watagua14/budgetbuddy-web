...
// Carga del gráfico solo en cliente (sin SSR)
...
export default function ClientApp() {
  ...
  const monthBudget = useMemo(() =>
    budgets.find(b => b.year === activeYearNum && b.month === activeMonthNum)?.amount || 0,
    [budgets, activeMonth]
  );

  const income = useMemo(() =>
    monthTxns.filter(t => (categories.find(c => c.id === t.categoryId)?.isIncome)).reduce((s, t) => s + t.amount, 0),
      ...
      <main className="max-w-3xl mx-auto px-4">
        ...
        <div className="flex items-center justify-center gap-2 py-4">
          ...
          <div className="font-medium">
            {new Date(activeYearNum, activeMonthNum - 1).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
          </div>
          <button className="pill" onClick={() => goMonth(1)}>▶︎</button>
        </div>
          ...
          {monthTxns.length === 0 ? (
            ...
            monthTxns.map(tx => (
              <TxnRow
                ...
                onEdit={() => setSheet({ type: 'edit-txn', id: tx.id })}
